...
function StatCounter({ end, label, delay = 0 }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '0 1.5rem' }}>
      <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 300, fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', color: '#fff', letterSpacing: '-0.02em' }}>
        {val.toLocaleString()}
      </div>
...
function CinematicReveal({ eyebrow, headline, body, imgSrc, reverse = false }) {
  return (
    <div style={{ display: 'flex', minHeight: '85vh', position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${imgSrc})`, backgroundSize: 'cover', backgroundPosition: 'center', zIndex: 0 }} />
      <div style={{ position: 'absolute', inset: 0, background: reverse ? 'linear-gradient(to left, rgba(10,10,10,0.82) 45%, rgba(10,10,10,0.0) 100%)' : 'linear-gradient(to right, rgba(10,10,10,0.82) 45%, rgba(10,10,10,0.0) 100%)', zIndex: 1 }} />
      <div style={{ position: 'relative', zIndex: 2, width: '50%', maxWidth: 560, padding: 'clamp(3rem, 8vw, 7rem) clamp(2rem, 5vw, 5rem)', display: 'flex', flexDirection: 'column', justifyContent: 'center', marginLeft: reverse ? 'auto' : 0 }}>
  ...
  return (
    <div style={{ background: DARK, color: CREAM, fontFamily: 'Georgia, serif', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${HERO_IMG})`, backgroundSize: 'cover', backgroundPosition: 'center', transform: `scale(1.08) translateY(${scrollY * 0.3}px)`, transition: 'transform 0.1s linear' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(10,10,10,0.35) 0%, rgba(10,10,10,0.55) 60%, rgba(10,10,10,0.88) 100%)' }} />
        <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 'clamp(3rem, 8vw, 6rem) clamp(3rem, 8vw, 6rem) 8rem' }}>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: 'clamp(3rem, 7vw, 6.5rem)', color: '#fff', lineHeight: 1.05, margin: '0 0 1.5rem', letterSpacing: '-0.03em', maxWidth: 900 }}>
            Garren Hill.<br /><em>A century</em><br />of belonging.
          </h1>
          <p style={{ color: CREAM, fontSize: 'clamp(0.95rem, 1.5vw, 1.15rem)', lineHeight: 1.7, maxWidth: 500, margin: '0 0 2.5rem', opacity: 0.8 }}>
            200 Hollycrest Drive -- 5 Beds -- 5 Baths -- 7 Fireplaces -- 4.15 Acres
          </p>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <a href="https://my.matterport.com/show/?m=mfwyqT5Btwx&brand=0&mls=1&" target="_blank" rel="noreferrer"
              style={{ padding: '0.85rem 2.2rem', background: 'transparent', border: '1px solid rgba(255,255,255,0.3)', color: 'rgba(255,255,255,0.75)', fontFamily: 'sans-serif', fontSize: '11px', letterSpacing: '0.25em', textTransform: 'uppercase', textDecoration: 'none', borderRadius: 2, display: 'inline-flex', alignItems: 'center' }}>
              Virtual Tour
            </a>
          </div>
        </div>
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 3, background: 'rgba(10,10,10,0.75)', backdropFilter: 'blur(10px)', borderTop: '1px solid rgba(255,255,255,0.08)', padding: '1.5rem 0', display: 'flex', justifyContent: 'center' }}>
          {[{ end: 1916, label: 'Year Built' }, { end: 6700, label: 'Sq Ft' }, { end: 7, label: 'Fireplaces' }, { end: 4, label: 'Acres' }].map((s, i) => (
      ...
      <div style={{ padding: 'clamp(5rem, 10vw, 9rem) clamp(2rem, 8vw, 10rem)', maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '1rem', lineHeight: 1.9, fontStyle: 'italic' }}>
      ...
      <div style={{ padding: 'clamp(4rem, 8vw, 7rem) clamp(2rem, 8vw, 12rem)', background: '#0f0f0f', textAlign: 'center' }}>
        <p style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 3vw, 2.8rem)', color: '#fff', lineHeight: 1.35, margin: '0 auto', maxWidth: 860 }}>
        ...
        <p style={{ fontFamily: 'sans-serif', fontSize: '10px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', margin: '1.5rem 0 0' }}>Betty Dumaine -- Owner 1959 to 1980</p>
      </div>
      ...
      <div style={{ padding: 'clamp(4rem, 8vw, 7rem) clamp(2rem, 6vw, 8rem)', background: DARK }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <span style={{ ...eyebrowStyle, textAlign: 'center', display: 'block', marginBottom: '2.5rem' }}>By the Numbers</span>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.06)' }}>
            {[
              { n: '5', label: 'Bedrooms' }, { n: '5', label: 'Bathrooms' },
              { n: '7', label: 'Fireplaces' }, { n: '4.15', label: 'Acres' },
              { n: '6,700', label: 'Sq Ft Heated' }, { n: '1916', label: 'Year Built' },
              { n: '3', label: 'Year Restoration' }, { n: '110+', label: 'Years of History' },
            ].map(item => (
              <div key={item.label} style={{ padding: '2.5rem 1.5rem', background: DARK, textAlign: 'center' }}>
      ...
      <CinematicReveal
        ...
        body="The original oak flooring is still here. So are the fireplace mantels -- all seven, preserved through the full 1999 restoration. Master carpenter David Prest rebuilt the Georgian woodwork from scratch: custom paneling, crown moldings, chair rails. The bones are 1916. The finish is flawless."
        imgSrc={LIVING_FIRE}
        reverse={true}
      />
      ...
      <div style={{ padding: 'clamp(5rem, 10vw, 9rem) clamp(2rem, 8vw, 10rem)', maxWidth: 820, margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ color: CREAM, fontSize: '1.05rem', lineHeight: 2, opacity: 0.85, margin: '2rem 0' }}>
          In 1998, Dr. and Mrs. Russell McAllister commissioned Durham architect Thomas O'Shea to recreate the original drawings from scratch. General contractor Dennis Dunagan led a three-year full restoration -- interior completely gutted, all plumbing and electrical replaced, new joists, every Marvin window custom-made.
        </p>
        <p style={{ color: CREAM, fontSize: '1.05rem', lineHeight: 2, opacity: 0.85 }}>
      ...
      <CinematicReveal
        ...
        body="Betty Dumaine planted three American sycamores in 1959. The McAllisters added holly trees, azaleas, dwarf nandinas, and iris beds. The brick terrace behind the house -- accessible from both the dining room and the living room -- seats a large group for dinner. There is a children's playhouse in the back. The locals call it The Wee Cottage."
        imgSrc={EXTERIOR_3}
      />
      ...
      <div style={{ padding: 'clamp(5rem, 10vw, 9rem) clamp(2rem, 8vw, 10rem)', background: '#0f0f0f' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          {[
            { year: '1916', title: 'Walter Hines Page', body: 'US Ambassador to the Court of St. James. Co-founder of Doubleday, Page and Co. The home was built for him. He never lived here.' },
            ...
            <div key={item.year} style={{ display: 'flex', gap: '2.5rem', marginBottom: '3rem', alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, borderLeft: `1px solid rgba(201,169,110,0.25)`, paddingLeft: '2rem' }}>
                <div style={{ fontFamily: 'Georgia, serif', fontSize: '1.05rem', color: '#fff', marginBottom: '0.5rem' }}>{item.title}</div>
      ...
      {/* CINEMATIC -- PRIMARY SUITE */}
      <CinematicReveal
        eyebrow="The Primary Suite"
        headline="Light from<br /><em>three directions.</em>"
        body="The NE/SW orientation the architect specified in 1916 still delivers. The back of the house is filled with windows. A balcony off two of the second-floor bedrooms overlooks the tennis courts and the rear grounds, where the local deer herd makes regular appearances."
        imgSrc={MASTER_BED}
        reverse={true}
      />
      ...
      {/* FEATURE CARDS */}
      <div style={{ padding: 'clamp(4rem, 8vw, 7rem) clamp(2rem, 6vw, 8rem)', background: DARK }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.5rem' }}>
          {[
            { eyebrow: 'Recognition', title: 'VHF Recognized Historic Property', body: 'One of the distinctly historical homes in southern Moore County. Virginia Historic Foundation recognized.' },
            { eyebrow: 'Infrastructure', title: 'County Water, Sewer + Private Well', body: 'County water and sewer lines. In-ground well and storage tank for irrigation and the pool. 250-gallon underground propane. 7 propane fireplaces.' },
            { eyebrow: 'Entertaining', title: 'Designed for Gatherings', body: 'Over 25 years as a venue for the English Speaking Union, Pi Beta Phi, Phi Beta Kappa, DAR, the Folio Club of Durham, and wedding celebrations.' },
            { eyebrow: 'Storage', title: 'Four-Room Stone Basement', body: 'Stone-walled basement under half the house. Partially finished, climate-controlled. Wine rack, full refrigerator, upright freezer, storage shelving. Interior and exterior access.' },
          ].map(card => (
            <div key={card.title} style={{ background: GLASS, border: `1px solid ${GLASS_BORDER}`, padding: '2.5rem 2rem' }}>
              ...
              <div style={{ fontFamily: 'Georgia, serif', fontSize: '1.05rem', color: '#fff', marginBottom: '0.75rem', lineHeight: 1.3 }}>{card.title}</div>
      ...
      <div style={{ padding: 'clamp(5rem, 10vw, 9rem) clamp(2rem, 8vw, 10rem)', textAlign: 'center', maxWidth: 700, margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: 'clamp(2rem, 3vw, 2.8rem)', color: '#fff', lineHeight: 1.2, margin: '0 0 2rem' }}>
          Minutes from<br /><em>Pinehurst Resort.</em>
        </h2>
        <div style={dividerStyle} />
        <p style={{ color: CREAM, fontSize: '1.05rem', lineHeight: 2, opacity: 0.85, margin: '2rem 0' }}>
          200 Hollycrest Drive sits in southern Moore County -- minutes from Pinehurst No. 2, the golf capital of the world. The Village of Pinehurst. The Sandhills community. All of it nearby without being in the middle of it.
        </p>
      </div>
      ...
      <div id="inquire" style={{ padding: 'clamp(5rem, 10vw, 9rem) clamp(2rem, 8vw, 10rem)', background: '#0f0f0f', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: 'clamp(2rem, 3vw, 2.8rem)', color: '#fff', lineHeight: 1.2, margin: '0 0 1rem' }}>Request Information</h2>
        <p style={{ color: CREAM, fontSize: '1rem', opacity: 0.6, margin: '0 0 3rem' }}>Offered at $4,250,000. Shown by appointment only.</p>
        {submitted ? (
      ...
      {/* MODAL */}
      {inquiryOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(10,10,10,0.92)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}
          ...
          <div style={{ background: '#141414', border: `1px solid ${GLASS_BORDER}`, padding: '3rem', maxWidth: 520, width: '100%' }}>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <button type="submit" style={{ padding: '0.9rem', background: GOLD, border: 'none', color: DARK, fontFamily: 'sans-serif', fontSize: '11px', letterSpacing: '0.3em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 600 }}>Submit</button>
              </form>
            )}
          </div>
        </div>

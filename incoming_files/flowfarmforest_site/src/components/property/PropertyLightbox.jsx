...
export default function PropertyLightbox({ galleries, group, index, onClose, onNavigate }) {
  ...
  }, [group, item, currentIndex, items.length]);
  ...
  const handleNext = () => {
    ...
    trackInteractionEvent('gallery_lightbox_navigated', {
      group: group || null,
      direction: 'next',
      image_alt: items[newIndex]?.alt || null,
      position: newIndex + 1,
    });
    onNavigate(newIndex);
  };

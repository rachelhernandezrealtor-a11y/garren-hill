          ...
          <div>
            {STRUCTURES.map((s, i) => (
              <div key={s.label} onClick={() => setActive(i)} style={{
                padding: '1.25rem 1.5rem', cursor: 'pointer', borderLeft: `3px solid ${active === i ? GOLD : 'transparent'}`,
                background: active === i ? 'rgba(191,162,116,0.05)' : 'transparent',
                marginBottom: '0.5rem', transition: 'all 0.2s',
              }}>
                <p style={{ color: 'rgba(191,162,116,0.7)', fontSize: '0.62rem', letterSpacing: '0.25em', textTransform: 'uppercase', fontFamily: 'sans-serif', marginBottom: '0.3rem' }}>{s.tag}</p>
                <p style={{ color: active === i ? '#fff' : 'rgba(255,255,255,0.55)', fontFamily: 'Georgia, serif', fontSize: '1.05rem' }}>{s.label}</p>
                <p style={{ color: GOLD, fontSize: '0.72rem', fontFamily: 'sans-serif', marginTop: '0.2rem' }}>{s.sf}</p>
              </div>
            ))}
          </div>
          <div>
            <img src={STRUCTURES[active].img} alt={STRUCTURES[active].label} style={{ width: '100%', height: 380, objectFit: 'cover', display: 'block', marginBottom: '2rem' }} />
            <h3 style={{ color: '#fff', fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: '1.6rem', marginBottom: '1rem' }}>{STRUCTURES[active].label}</h3>
            <p style={{ color: 'rgba(255,255,255,0.65)', lineHeight: 1.9, fontSize: '0.95rem', fontFamily: 'sans-serif' }}>{STRUCTURES[active].desc}</p>
          </div>
        </div>
      </div>
    </section>
